// Implementação da API para gerenciar contas do Google Ads
...
// Handler para GET /api/google/accounts
...
// Handler para POST /api/google/accounts
  ...
  try {
    ...
    const accountCheck = await db.execute(
      sql`SELECT id FROM ad_accounts 
          WHERE platform = 'google' AND account_id = ${body.account_id}`
    );
    
    if (accountCheck.results.length > 0) {
      return NextResponse.json({ 
        success: false, 
        error: 'Conta do Google Ads já cadastrada' 
      }, { status: 409 });
    }
    ...
    const result = await db.execute(
      sql`INSERT INTO ad_accounts (client_id, platform, account_id, account_name, status) 
          VALUES (${body.client_id}, 'google', ${body.account_id}, ${body.account_name}, ${body.status || 'active'}) 
          RETURNING *`
    );
    ...
    console.error('Erro ao criar conta do Google Ads:', error);
    return NextResponse.json({ 
      success: false, 
      error: 'Erro ao criar conta do Google Ads' 
    }, { status: 500 });
  }
}

// Handler para PUT /api/google/accounts/:id
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const accountCheck = await db.execute(
      sql`SELECT id FROM ad_accounts WHERE id = ${id} AND platform = 'google'`
    );
    ...
    updateQuery += ' WHERE id = ? AND platform = \'google\'';
    params.push(id);
    ...
    console.error('Erro ao atualizar conta do Google Ads:', error);
    return NextResponse.json({ 
      success: false, 
      error: 'Erro ao atualizar conta do Google Ads' 
    }, { status: 500 });
  }
}

// Handler para DELETE /api/google/accounts/:id
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    return NextResponse.json({ 
      success: true, 
      message: 'Conta do Google Ads excluída com sucesso' 
    });
  } catch (error) {
    console.error('Erro ao excluir conta do Google Ads:', error);
    return NextResponse.json({ 
      success: false, 
      error: 'Erro ao excluir conta do Google Ads' 
    }, { status: 500 });
  }
}
